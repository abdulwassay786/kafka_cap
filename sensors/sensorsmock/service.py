...
class SensorService:
    ...
    #     df.to_csv(f"s3://{self._smart_thermo_bucket}/lux_meter/data.csv")
    #     return data1

    # @app.get("/data")
    # async def get_luxmeter(self, room_id: str):
    #     if not self.is_allowed_room(room_id):
    #         return {"error": f"Room {room_id} not exists!"}
    #     data = self.get_lux_meter_data(room_id)
    #     room_id = "kitchen"
    #     df = pd.DataFrame.from_dict(data, orient='index')
    #     df.to_csv(f"s3://{self._smart_thermo_bucket}/lux_meter/data_2.csv")
